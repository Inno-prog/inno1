...
export async function GET(request: Request) {
  try {
    ...
    const [rows] = await select('SELECT * FROM stagiaires WHERE id = ?', [Number(session.user.id)]);
...
export async function PUT(request: Request) {
  try {
    ...
    const result = await modify(
      ...
      [nom, prenom, email, telephone, etablissement, filiere, niveau_etude, Number(session.user.id)]
    );
